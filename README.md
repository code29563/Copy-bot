...
- You can maintain multiple copying streams, with a destination channel receiving messages from multiple source channels and copying messages from a source channel to multiple destination channels.
- Messages are copied to the destination channel in the same order in which they were sent to the source channel.
- All new messages that are sent to the source channel get copied to the destination channel, even if a large number were sent at once.
...
- When a message is edited in the source channel, it gets copied to the destination channel (with the caption indicating they are previous messages edited).
...
3. Fill in the STREAMS environment variable, which gives details of the source channels from which messages are to be copied and the destination channels to which they are copied. You can split the streams on multiple lines, and leave spaces between the IDs and the brackets/commas/semi-colons, if you want. The syntax is as follows:
	```
	STREAMS = "
	...
	You can find the IDs of the channels in various ways, like with [this script](https://github.com/code29563/List-Telegram-Chat-IDs), @username\_to\_id\_bot, or exporting your Telegram data from the app as 'Machine-readable JSON' where you can find the IDs of channels you're subcribed to in results.json (but be sure to append -100 to them before inserting them in STREAMS).
	
	Note that if a channel is a source for one channel and a destination for another, e.g.
	```
...
Based on the input the value of the environment variable STREAMS, the script makes a list of destination channels for each source channel.

The script obtains a list of all the [dialogs](https://core.telegram.org/constructor/dialog) and checks those that correspond to the source channels. From the dialog, the 'top message' of each source channel is recorded. This is the ID of the most recent still-existing message sent to the source channel. I specify 'still-existing' as the actual most recent message sent to a channel could have been deleted.

The userbot is run and receives updates of new messages sent in the the source channels or old messages edited. Assume that the top message of a particular source channel was the actual most recent message sent in it (i.e. it hadn't been deleted). The ID of the next new message sent there is expected to be the ID of that top message plus 1.

If the userbot receives an update of a message with an ID equal to this expected ID, then the message is copied to the destination channels with its caption, and the recorded ID of the top message is updated to be the ID of this new message, which as far as the script is concerned is now the most recent message in the source channel.

When a message is edited, it retains its original ID. If the update was due to an old message being edited, then its ID is less than the expected ID of the next new message (top message +1), which is expected knowing that it's an old message receiving an edit and it's not the new most recent message in the source channel, so this newly edited message is copied to the destination channels and the recorded ID of the top message is left at its current value and not updated.

If an update received is for a message with an ID greater than the expected ID, then it may be encountering a situation like what's mentioned above, where a large number of messages were forwarded to the source channel at once and the userbot receives the updates in the wrong order. In this case the userbot separately retrieves all messages with ID >= the expected ID, up to the most recent message, then copies them one by one to the destination channels. As each message gets copied, the recorded ID of the top message is updated to be its ID, (the reason for which is explained in the next section).

So in the above example, the userbot receives the update of message 80 first, which has an ID that's 79 greater than the expected ID (the ID of message 1), so the userbot retrieves messages all messages with ID >= the expected ID (that of message 1), up to the most recent message (which is message 100), and copies them one by one to the destination channels, updating the recorded ID of the top message each iteration.
...
# Other notes
Note that messages with buttons don't get copied. The text part of such a message may get copied but the buttons themselves don't.